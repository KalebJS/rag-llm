...
def fetch_embeddings(texts: list[str], embedding_type: str = "search_document") -> list[list[float]]:
    try:
        results = co.embed(texts=texts, model=COHERE_EMBEDDING_MODEL, input_type=embedding_type).embeddings
        return results
...
def synthesize_answer(question: str, context: list[str]) -> str:
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[{"role": "system", "content": question_and_answer_prompt(question, context)}],
        temperature=0,
    )
